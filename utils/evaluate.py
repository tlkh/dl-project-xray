...
#         for index in trange(len(dataset), dynamic_ncols=True):
        for index in progress_bar(range(len(dataset))):
            ...
            image_tensor = image_tensor.unsqueeze(0).to(config.device, non_blocking=True)
...
#         for index in trange(len(dataset), dynamic_ncols=True):
        for index in progress_bar(range(len(dataset))):
            ...
            image_tensor_frontal = image_tensor_frontal.unsqueeze(0).to(config.device, non_blocking=True)
            image_tensor_lateral = image_tensor_lateral.unsqueeze(0).to(config.device, non_blocking=True)
